...
/* 
I looked up "how to use sort() to order an array of objects by year" and Google's generic AI answer immediately gave me helpful results.
The suggested code was written in one line (I'm not sure the technical term for the syntax).
I rewrote it using the syntax we've been using in our modules and got the expected log.
Here are the notes from Google's AI Overview:
"It compares the year property of a and b.
"If a.year is less than b.year, it returns a negative number, indicating that a should come before b.
"If a.year is greater than b.year, it returns a positive number, indicating that b should come before a.
"If a.year is equal to b.year, it returns 0, indicating that the order doesn't matter."
*/
...
Hint: Use the String.prototype.() method to separate the first and last 
...
// I couldn't figure out how to combine everything below into one block of code...

const lifeSpan = inventors.map((inventor) => {
    return inventor.passed - inventor.year;
});

totalYearsLived = lifeSpan.reduce(function (acc, yearsLived) {
